...
function EdytujKategorie($kategoria_id, $nowa_nazwa, $nowa_matka){
    global $link;

    $query = "UPDATE kategorie SET nazwa = '$nowa_nazwa', matka = '$nowa_matka' WHERE id = '$kategoria_id'";
...
function EdytujKategorieForm($link) {
    ...
    <form method="post" action="' . $_SERVER['REQUEST_URI'] . '">
		...
		<label for="nowa_matka">Nowa Nazwa Matki:</label>
        <input type="text" id="nowa_matka" name="nowa_matka" required><br>
...
function EdytujKategorieAction($link) {
    if (isset($_POST['edytuj_kategorie'])) {
        if (isset($_POST['kategoria_id'], $_POST['nowa_nazwa'], $_POST['nowa_matka'])) {
            $kategoria_id = $_POST['kategoria_id'];
            $nowa_nazwa = $_POST['nowa_nazwa'];
			$nowa_matka = $_POST['nowa_matka'];
